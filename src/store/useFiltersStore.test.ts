...
const createTestStore = () => {
  ...
  beforeEach(() => {
    ...
    }, true); // true replaces the state entirely instead of merging
  });
...
describe('useFiltersStore', () => {
  ...
  it('should reset filters correctly', () => {
    ...
    store.getState().setMinimumRating(4);
    store.getState().setUserLocation({ lat: 40.7128, lng: -74.0060 });
    ...
    expect(store.getState().userLocation).toEqual({ lat: 40.7128, lng: -74.0060 });
    expect(store.getState().viewport.center).toEqual({ lat: 37.7749, lng: -122.4194 });
