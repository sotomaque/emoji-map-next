...
import { MapPin, Star, Image as ImageIcon, MessageSquare, Store } from "lucide-react"
...
import type { AdminSearchResult, AdminSearchResponse } from "@/types/admin-search"
...
type DashboardAction =
  ...
  | { type: "START_LOADING" }
  | { type: "UPDATE_LOADING_STEP"; payload: { index: number; status: LoadingStep["status"] } }
...
const initialState: DashboardState = {
  dialogState: "closed",
  loadingSteps: [
    { message: "Finding your business", status: "pending" },
  ],
  isSubmitting: false,
  searchResults: [],
  selectedPlace: null,
...
export default function MerchantDashboard() {
  ...
  const { data: merchantData, isLoading: isLoadingMerchant } = useQuery<MerchantResponse>({
    queryKey: ['merchant'],
    queryFn: async () => {
  ...
  const associatePlaceMutation = useMutation<
    AssociateResponse,
    Error,
    string
  >({
  ...
  async function onSubmit(data: ClaimPlaceFormValues) {
    try {
      ...
      if (searchResult.data?.length === 0) {
        ...
        toast.error("No matching places found. Please check your business details.")
        return
      }

      // Show list of places
      dispatch({ type: "SHOW_LIST", payload: { searchResults: searchResult.data } })
      form.reset()
  ...
  async function onVerifySubmit(data: VerifyPhoneFormValues) {
    try {
      ...
      const placeNumber = stripNonNumeric(state.selectedPlace?.nationalPhoneNumber || '')

      if (inputNumber === placeNumber) {
        // Call the associate endpoint
        const result = await associatePlaceMutation.mutateAsync(state.selectedPlace!.id)
        if (result.success) {
              ...
              {isLoadingMerchant ? (
                ...
                <div className="text-2xl font-bold">
                  {merchantData?.merchant?.places?.length ?
                    (merchantData.merchant.places.reduce<number>((acc, place) =>
                      acc + (place.ratings.reduce<number>((sum, r) => sum + r.rating, 0) / (place.ratings.length || 1)), 0)
                      / merchantData.merchant.places.length).toFixed(1)
                    : '--'}
                </div>
              )}
          ...
          <CardContent>
            <div className="flex items-center justify-between">
              {isLoadingMerchant ? (
                ...
                <div className="text-2xl font-bold">
                  {merchantData?.merchant?.places?.reduce<number>((acc, place) =>
                    acc + (place.reviews?.length || 0), 0) || 0}
                </div>
              )}
              ...
              <CardContent className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-2">
                      <MessageSquare className="h-4 w-4 text-muted-foreground" />
                      <p className="text-sm text-muted-foreground">{place.reviews.length} reviews</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <ImageIcon className="h-4 w-4 text-muted-foreground" />
                      <p className="text-sm text-muted-foreground">{place.photos.length} photos</p>
                    </div>
                  </div>
            ...
            <CardContent className="p-6">
              <div className="text-center py-6">
                ...
                <p className="text-muted-foreground mb-4">
                  You haven&apos;t claimed any places yet. Start by claiming your first business.
                </p>
                <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Start managing your business by claiming ownership of your Google Places listing
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Dialog
              open={state.dialogState === "claim"}
              onOpenChange={(open) => dispatch(open ? { type: "OPEN_CLAIM_DIALOG" } : { type: "CLOSE_DIALOG" })}
            >
              ...
              <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Fill out this form to start the process of claiming your business on Emoji Map.
                  </DialogDescription>
                </DialogHeader>
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-4 pt-4">
                      ...
                      <Button
                        type="submit"
                        disabled={state.isSubmitting || searchPlaceMutation.isPending}
                      >
                        {state.isSubmitting || searchPlaceMutation.isPending ? "Submitting..." : "Submit"}
                      </Button>
                    </div>
                  </form>
                </Form>
                    ...
                    <div key={index} className="flex items-center gap-4">
                      <div className="flex-1">
                        {step.status === "pending" ? (
                          ...
                          <p className={step.status === "error" ? "text-destructive" : "text-muted-foreground"}>
                            {step.message}
                          </p>
                        )}
                      </div>
                  ...
                  <div className="space-y-4">
                    {state.searchResults.map((place) => (
                      ...
                      >
                        ...
                        <Button
                          onClick={() => dispatch({ type: "SELECT_PLACE", payload: place })}
                        >
                          Select
                        </Button>
                      </div>
            ...
            >
              <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Please enter the phone number associated with {state.selectedPlace?.displayName} to verify ownership.
                  </DialogDescription>
                </DialogHeader>
                <Form {...verifyForm}>
