...
// TODO: should be able to reuse my types here
...
export async function fetchPlaces(
  ...
  if (categoryKeys && categoryKeys.length > 0) {
    ...
    const validKeys = categoryKeys.filter(
      (key) => typeof key === 'number' && key >= 1 && key <= 25
    );
 ...
 */
export function placesToGoogleMapsMarkers(
