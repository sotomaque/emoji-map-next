...
export default function GoogleMapComponent({
  ...
  useEffect(() => {
    if (!apiKey) {
      ...
      console.warn('Google Maps API key seems too short. Please check your NEXT_PUBLIC_GOOGLE_MAPS_API_KEY environment variable.');
    }
  }, [apiKey]);
  ...
  const handleCenterChanged = useCallback(() => {
    if (mapRef.current && onCenterChanged) {
      ...
      if (newCenter) {
        ...
        if (Math.abs(centerObj.lat - center.lat) > 0.0001 ||
    ...
    return (
      <div className="flex items-center justify-center h-full bg-gray-100 dark:bg-gray-900">
        <div className="text-center p-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">{loadError.message}</p>
        </div>
      </div>
    );
  }
          ...
          >
            <div className="info-window">
              ...
              <div className="info-window-category">
                <span className="info-window-emoji">{selectedMarker.emoji}</span>
